...
import ReadOnlyRecordView from '../components/ReadOnlyRecordView';
...
export default function MedicalRecords() {
  ...
  useEffect(() => {
    ...
    socket.on('record_updated', load);
    return () => {
      socket.off('record_added', load);
      socket.off('record_updated', load);
    };
  // eslint-disable-next-line
  }, [socket]);
    ...
    <div>
      ...
      {loading ? (
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
          {filtered.map(r => (
            <div key={r._id} className="card" style={{ cursor: 'pointer', padding: '18px 20px' }} onClick={() => handleRecordClick(r)}>
      ...
      {selectedRecord && user.role === 'patient' && (
        <ReadOnlyRecordView 
      ...
      {/* Editable view for doctors/admins */}
      {selectedRecordForEdit && (user.role === 'doctor' || user.role === 'admin') && (
        <EditRecordModal 
...
function EditRecordModal({ record, patients, onClose, onUpdated, api }) {
  const [form, setForm] = useState({
    patient_id: record.patient_id?._id || '',
    date: record.date ? new Date(record.date).toISOString().split('T')[0] : new Date().toISOString().split('T')[0],
    diagnosis: record.diagnosis || '',
    symptoms: record.symptoms?.join(', ') || '',
    other_records: record.other_records || '',
    is_emergency: record.is_emergency || false,
    vital_signs: record.vital_signs || { blood_pressure: '', pulse: '', temperature: '', weight: '', height: '', oxygen_level: '' },
    prescriptions: record.prescriptions || [{ medicine: '', dosage: '', frequency: '', duration: '', notes: '' }],
  });
  const [loading, setLoading] = useState(false);
    ...
    <div className="modal-overlay" onClick={onClose}>
      <div className="modal" style={{ maxWidth: 680 }} onClick={e => e.stopPropagation()}>
        ...
        <form onSubmit={submit}>
          ...
          <div className="modal-footer">
            <button type="button" className="btn btn-outline" onClick={onClose}>Cancel</button>
            <button type="submit" className="btn btn-primary" disabled={loading}>{loading ? 'Updating…' : 'Update Record'}</button>
          </div>
        </form>
      </div>
    </div>
